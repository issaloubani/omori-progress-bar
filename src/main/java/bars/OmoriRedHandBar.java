...
public class OmoriRedHandBar extends BasicProgressBarUI implements IOmoriBar {
    ...
    protected void paintIndeterminate(Graphics g, JComponent c) {
        // Keep the sprite fully on screen: its centre travels between half an icon
        ...
        paintBar(g, c, c.getWidth(), half + Math.round(bounce * travel), p < 0.5f);
    }
    ...
    protected void paintDeterminate(Graphics g, JComponent c) {
        ...
        paintBar(g, c, filled, filled, true);
    }
    ...
    /**
     * @param fillWidth     how much of the track is coloured in, px
     * @param spriteCentreX where the sprite's centre sits, px from the left
     * @param facingRight   sprite orientation
     */
    private void paintBar(Graphics graphics, JComponent c, int fillWidth, int spriteCentreX, boolean facingRight) {
        int w = c.getWidth();
        ...
        try {
            ...
            // Centre vertically - everything below is drawn in "bar space".
            ...
            g.setPaint(trackPaint(h));
            g.fill(new RoundRectangle2D.Float(0, 0, Math.max(fillWidth, arc), h, arc, arc));

            // Centre the sprite on both axes from its real size, so swapping in a
            // different icon never needs the offsets retuned.
            paintSprite(g, c,
                    spriteCentreX - HAND.getIconWidth() / 2,
     ...
     */
    protected Paint trackPaint(int height) {
        return new JBColor(Gray._30, Gray._220);   // dark on light theme, light on dark;
    }
